...
class KasirController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('kasir.create', compact('products', 'users'));
    }
    ...
    public function edit($id)
    {
        ...
        $products = Product::where('status', 'in stock')->get();
        $users = User::all();
        return view('kasir.edit', compact('sale', 'products', 'users'));
    }

    // Menangani proses update penjualan
    public function update(Request $request, $id)
    {
        ...
        $request->validate([
            ...
            'products.*.quantity' => [
                function ($attribute, $value, $fail) use ($request) {
                    $productId = $request->input('products.' . explode('.', $attribute)[1] . '.id');
                    $product = Product::find($productId);
                    if ($product->stok < $value) {
                        $fail('Stok produk ' . $product->name . ' tidak cukup.');
                    }
    ...
    public function exportPdf()
    {
        ...
        // Generate PDF
        $pdf = Pdf::loadView('kasir.details_pdf', compact('sales'));
